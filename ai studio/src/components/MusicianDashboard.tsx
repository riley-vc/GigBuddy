...
import { Calendar, DollarSign, Hourglass, CheckSquare, Music, Plus, X, ToggleLeft, ShieldAlert } from 'lucide-react';
import { MusicianProfile, Gig, Application, MoaContract } from '../types';
...
interface MusicianDashboardProps {
  ...
  onUpdateAvailability: (day: string, status: 'available' | 'busy' | 'tentative') => void;
...
export default function MusicianDashboard({
  ...
  // Calculate Musician Pipeline stats
  // Confirmed bookings are gigs where status is filled and this user is on a signed contract
  const myGigs = contracts.filter((c) => c.status === 'fully_signed');
  const upcomingCount = myGigs.length;

  const myApplications = applications.filter((a) => a.musicianId === profile.id);
  const pendingCount = myApplications.filter((a) => a.status === 'pending').length;

  // Earnings are sum of compensation for fully signed & completed contracts
  const projectedEarnings = contracts.reduce((acc, curr) => acc + curr.compensation, 0);

  const handleAddBandSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (newBandName.trim()) {
  ...
  const cycleAvailability = (day: string, currentStatus: 'available' | 'busy' | 'tentative') => {
    let nextStatus: 'available' | 'busy' | 'tentative' = 'available';
  ...
  return (
    <div id="musician-dashboard-wrapper" className="space-y-6">
      ...
      <div id="musician-stats-row" className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl flex items-center justify-between shadow-sm relative overflow-hidden">
          <div className="space-y-1">
            ...
            <span className="text-3xl font-display font-extrabold text-zinc-50 block">{upcomingCount}</span>
            <span className="text-[10px] text-zinc-400 block">Fully contracted contracts</span>
          </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl flex items-center justify-between shadow-sm relative overflow-hidden">
          <div className="space-y-1">
            ...
            <span className="text-3xl font-display font-extrabold text-amber-400 block">{pendingCount}</span>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl flex items-center justify-between shadow-sm relative overflow-hidden">
          <div className="space-y-1 text-zinc-50">
            ...
            <span className="text-3xl font-display font-extrabold block text-emerald-400">${projectedEarnings}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left Side: Availability & Bands */}
        <div className="lg:col-span-7 space-y-6">
          {/* Multi-Band Availability Tracker */}
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 shadow-md">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-zinc-800">
              <div>
                <h3 className="font-display font-bold text-zinc-50 text-sm">Multi-Band Availability Tracker</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2">
              {Object.entries(profile.availability).map(([day, status]) => (
                <button
                  ...
                  className="p-3 rounded-lg border flex flex-col items-center justify-center gap-1.5 cursor-pointer text-center group transition-all active:scale-95"
                >
            ...
            <div className="flex justify-center gap-4 mt-4 pt-4 border-t border-zinc-800/60 text-[10px] text-zinc-500">
              <span className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> Available (Ready to Book)
              </span>
              <span className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-red-400" /> Busy (Gig/Practice locked)
              </span>
              <span className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-400" /> Tentative (Pending Hold)
              </span>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 shadow-md">
            <h3 className="font-display font-bold text-zinc-50 text-sm mb-1">Live Band & Project Registry</h3>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {profile.bands.map((band) => (
                  <span 
                    key={band} 
                    className="px-2.5 py-1.5 bg-zinc-950 border border-zinc-850 rounded-lg text-xs text-zinc-300 flex items-center gap-1.5"
                  >
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 shadow-md relative overflow-hidden">
            <div className="flex gap-3.5 items-center pb-4 border-b border-zinc-800">
              ...
              />
              <div>
                <h4 className="font-display font-extrabold text-zinc-50 text-base">{profile.name}</h4>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {profile.skills.map((skill) => (
                    <span key={skill} className="px-2 py-0.5 bg-zinc-950 border border-zinc-850 text-[10px] text-zinc-400 rounded-md font-mono">
                      ✓ {skill}
                    </span>
                  ))}
                </div>
            ...
            <div className="bg-zinc-950 p-3.5 rounded-lg border border-zinc-850 flex gap-2.5 items-start">
              <CheckSquare className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
              <p className="text-[11px] text-zinc-400 leading-relaxed">
                <strong className="text-zinc-200">Payment Security Note:</strong> Your verified skills tag matches automatic search queries. Complete all gigs as scheduled to receive prompt escrow settlement.
              </p>
            </div>
          </div>
        </div>
