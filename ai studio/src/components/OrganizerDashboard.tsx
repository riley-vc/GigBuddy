...
import { Gig, Application, MoaContract } from '../types';

interface OrganizerDashboardProps {
  gigs: Gig[];
  applications: Application[];
  contracts: MoaContract[];
  onApproveApplication: (appId: string) => void;
  onRejectApplication: (appId: string) => void;
  onCancelGig: (gigId: string) => void;
  onOpenContract: (contract: Partial<MoaContract>) => void;
}
...
export default function OrganizerDashboard({
  ...
  const [activeTab, setActiveTab] = useState<'managed' | 'applicants' | 'contracts'>('managed');
  ...
  return (
    <div id="organizer-dashboard-wrapper" className="space-y-6">
      {/* 1. High-Level Metrics Trackers */}
      <div id="organizer-metrics-row" className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Metric 1 */}
        <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl flex items-center justify-between shadow-sm relative overflow-hidden">
          <div className="space-y-1">
            <span className="text-zinc-500 text-xs font-semibold uppercase tracking-wider block">Active Open Calls</span>
            <span className="text-3xl font-display font-extrabold text-zinc-50 block">{activeOpenCalls}</span>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl flex items-center justify-between shadow-sm relative overflow-hidden">
          <div className="space-y-1 text-zinc-50">
            ...
            <span className="text-3xl font-display font-extrabold block text-amber-400">{pendingApps}</span>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl flex items-center justify-between shadow-sm relative overflow-hidden">
          <div className="space-y-1">
            ...
            <span className="text-3xl font-display font-extrabold text-emerald-400 block">{confirmedBookings}</span>
      ...
      <div id="organizer-tabs-nav" className="flex border-b border-zinc-800">
        <button
          ...
          className={`px-5 py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors cursor-pointer ${
            activeTab === 'managed'
              ? 'border-violet-600 text-zinc-50'
              : 'border-transparent text-zinc-400 hover:text-zinc-200'
          }`}
        >
          My Events ({gigs.length})
        </button>
        <button
          id="btn-tab-applicants"
        ...
        >
          MoA Agreements ({contracts.length})
        </button>
      </div>
      ...
      <div id="organizer-tab-panel" className="space-y-4">
        ...
        {activeTab === 'managed' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-display font-semibold text-zinc-100 text-sm">Managed Events Timeline</h3>
              <span className="text-[11px] font-mono text-zinc-500">Updated seconds ago</span>
            </div>
            ...
            {gigs.length === 0 ? (
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center space-y-3">
                ...
                <p className="text-xs text-zinc-500">Switch to the "Publish Open Gig Call" tab to generate your first event!</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {gigs.map((gig) => {
                  ...
                  return (
                    <div 
                      key={gig.id} 
                      className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-colors flex flex-col justify-between space-y-4"
                    >
                      <div className="space-y-2">
                        ...
                        <div>
                          <h4 className="font-display font-bold text-zinc-100 text-base line-clamp-1">{gig.title}</h4>
                        ...
                        {/* Technical Tag summaries */}
                        <div className="flex flex-wrap gap-1 pt-1">
                          {gig.instruments.map((i) => (
                            <span key={i} className="px-1.5 py-0.5 bg-zinc-950 text-[9px] font-mono text-zinc-400 rounded">
                              {i}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="pt-4 border-t border-zinc-800/60 flex items-center justify-between text-xs">
                        <span className="text-zinc-400">
                          <strong>{gigApps.length}</strong> applicants ({activeAppCount} pending review)
                        </span>
        ...
        {activeTab === 'applicants' && (
          <div className="space-y-4">
            <h3 className="font-display font-semibold text-zinc-100 text-sm">Musician Applications Awaiting Review</h3>
            ...
            {pendingApps === 0 ? (
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center space-y-2">
                ...
                <p className="text-xs text-zinc-500">Wait for sessionists to apply to your open calls in the marketplace, or publish more calls!</p>
              </div>
            ) : (
              <div className="space-y-4">
                {applications
                  ...
                  .map((app) => {
                    const associatedGig = gigs.find((g) => g.id === app.gigId);
                    if (!associatedGig) return null;

                    return (
                      <div 
                        key={app.id} 
                        className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 hover:border-zinc-800/80 transition-all flex flex-col md:flex-row gap-5 justify-between"
                      >
                        {/* Profile Details */}
                        <div className="flex gap-4 items-start flex-1">
                          ...
                          <div className="space-y-2 flex-1">
                            <div>
                              <h4 className="font-display font-bold text-zinc-100 text-base flex items-center gap-2">
                                {app.musicianName}
                            ...
                            <div className="flex flex-wrap gap-1">
                              {app.skills.map((skill) => (
                                <span key={skill} className="px-2 py-0.5 bg-zinc-950 text-[10px] font-mono text-zinc-400 rounded">
                                  {skill}
                                </span>
                              ))}
                            </div>
                            ...
                            {/* Cover Letter */}
                            <div className="bg-zinc-950/60 p-3.5 rounded-lg border border-zinc-850 text-xs text-zinc-300 leading-relaxed italic">
                              "{app.coverNote}"
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex md:flex-col justify-end gap-2 shrink-0 border-t md:border-t-0 border-zinc-800 pt-3 md:pt-0">
                          ...
                          <div className="flex gap-2 w-full md:w-auto">
                            <button
                              ...
                              className="flex-1 md:flex-none px-3.5 py-2 bg-zinc-950 hover:bg-zinc-800 border border-zinc-850 text-zinc-400 rounded-lg text-xs font-semibold transition-colors flex items-center justify-center gap-1.5"
                            >
        ...
        {/* C. MoA Agreements (Archived and live contracts) */}
        {activeTab === 'contracts' && (
          <div className="space-y-4">
            <h3 className="font-display font-semibold text-zinc-100 text-sm">Memorandums of Agreement & Escrow States</h3>
            ...
            {contracts.length === 0 ? (
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center space-y-1">
                ...
                <p className="text-xs text-zinc-500">Approve candidate applications to draft legally-binding standard performance agreements.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-3">
                {contracts.map((contract) => (
                  <div 
                    key={contract.id} 
                    className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4"
                  >
                    <div className="flex items-center gap-3.5">
                      ...
                      <div>
                        <h4 className="font-display font-bold text-zinc-100 text-sm">{contract.gigTitle}</h4>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-zinc-800/60 pt-3.5 md:pt-0">
                      <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold font-mono ${
                        ...
                        {contract.status === 'fully_signed' ? 'Fully Signed (Escrow Active)' : contract.status === 'completed' ? 'Archived (Completed)' : 'Awaiting Signatures'}
                      </span>

                      <button
                        id={`open-contract-${contract.id}`}
                        onClick={() => onOpenContract(contract)}
                        className="px-3 py-1.5 bg-zinc-950 hover:bg-zinc-800 text-zinc-300 rounded border border-zinc-850 text-xs font-semibold transition-colors flex items-center gap-1.5"
                      >
